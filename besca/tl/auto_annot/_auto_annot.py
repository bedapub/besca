...
def report(
    ...
    report = classification_report(
        adata_pred.obs[celltype], adata_pred.obs["auto_annot"], output_dict=True
    )
    ...
    with open("auto_annot_report_" + analysis_name + ".csv", mode="w") as report:
        ...
        sc.pl.umap(
            adata_pred,
            color=[celltype, "auto_annot", clustering],
            legend_loc="on data",
            legend_fontsize=7,
        ...
        class_names = np.unique(
            np.concatenate((adata_pred.obs[celltype], adata_pred.obs["auto_annot"]))
        )
...
def plot_confusion_matrix(
    ...
    if asymmetric_matrix == True:
        class_names = np.unique(
            np.concatenate(
                (adata_predicted.obs[celltype], adata_predicted.obs["auto_annot"])
            )
        )
