...
def extract_info_rank_genes_groups(adata):
    ...
    for i in range(1,group_number):
        n_FDRs = DataFrame({'n_' + key[:1]: rank_genes[key][groups [i]] for key in ['names', 'pvals_adj']})
...
def perform_dge(adata,
    ...
    for i in range(design_matrix.shape[0]):
        ...
        contrast = str(group1[1]+'_'+group1[0]+'_vs_'+group2[1]+'_'+group2[0])
        ...
        #get adata_subset
        adata_subset = subset_adata(adata_raw, filter_criteria=adata.obs.get(constant_criteria) == group1[1], raw = False)
        ...
        if counts.get(group1[0]) is None:
            ...
            print('not enough cells for comparison', contrast, ' in one of the groups')
            too_few_cells.append(i)
        elif not((counts.get(group1[0]) > min_cells_per_group) & (counts.get(group2[0]) > min_cells_per_group)):
            print('----------------------------------------')
            print('not enough cells for comparison', contrast, ' in one of the groups')
            too_few_cells.append(i)
        else:
            ...
            rank_genes_groups(adata=adata_subset, groupby=differentiating_criteria, reference=group2[0], groups = [group1[0]], method = method, n_genes= adata_subset.n_vars)
            ...
            mean_group1 = adata_subset[adata_subset.obs.get(differentiating_criteria) == group1[0]].X.todense().mean(axis = 0).tolist()[0]
            mean_group2 = adata_subset[adata_subset.obs.get(differentiating_criteria) == group2[0]].X.todense().mean(axis = 0).tolist()[0]

            #get mean expression
            log2cp10k = DataFrame(data = {'log2cp10k':mean_all,
            ...
            top_table = DataFrame(data = {'Contrast':contrast,
                                          'SYMBOL':scores.Description,
                                          'ENSEMBL':scores.index.tolist(),
                                          'Score':scores.get(group1[0])},
                                  index = scores.index)
            #add p-value, logFC, FDRs, mean_expression
            top_table = top_table.merge(pvalues.get(group1[0]).to_frame().rename(columns = {group1[0]:'P-value'}), how='left', right_index=True, left_index = True)
            top_table = top_table.merge(logFC.get(group1[0]).to_frame().rename(columns = {group1[0]:'logFC'}), how='left', right_index=True, left_index = True)
            top_table = top_table.merge(FDRs.get(group1[0]).to_frame().rename(columns = {group1[0]:'FDR'}), how='left', right_index=True, left_index = True)
            top_table = top_table.merge(log2cp10k, how='left', right_index=True, left_index = True)
            ...
            if method == 'wilcoxon':
                ...
                TopTable_File = os.path.join(outpath, "WilxTopTable_"+contrast+".txt")
            elif method == 't-test_overestim_var':
                rank_File = os.path.join(outpath, "OverestRank_"+contrast+".rnk")
                TopTable_File = os.path.join(outpath, "OverestTopTable_"+contrast+".txt")
            elif method == 't-test':
                rank_File = os.path.join(outpath, "tTestRank_"+contrast+".rnk")
                TopTable_File = os.path.join(outpath, "tTestTopTable_"+contrast+".txt")
            elif method == 'logreg':
                rank_File = os.path.join(outpath, "logregRank_"+contrast+".rnk")
                TopTable_File = os.path.join(outpath, "logregTopTable_"+contrast+".txt")
            else:
            ...
            #write out TopTable
            top_table.to_csv(TopTable_File, sep = '\t', index=True, header=True, float_format='%1.3e')
    ...
    if len(too_few_cells) > 0:
        ...
        print('')
        print('----------------------------------------------')
        print('the following comparisons from the design_matrix could not per performed due to too few cells')
        display(design_matrix.iloc[too_few_cells,:])

    return(None)
    sys.exit(0)

def plot_interactive_volcano(top_table_path,
                             outdir):
    ...
    def generate_data_trace(df, threshold):
        ...
        expression_group1 = ['log2cp10k of '+group1_label+': '+str(expr) +'<br>' for expr in df_filtered.get(group1).tolist()]
        expression_group2 = ['log2cp10k of '+group2_label+': '+str(expr) +'<br>' for expr in df_filtered.get(group2).tolist()]
        pvalue = ['P-value: '+str(expr) +'<br>' for expr in df_filtered.FDR.tolist()]
        foldchange = ['logFC: '+str(value) +'<br>' for value in df_filtered.logFC.tolist()]
        text = [a+b+c+d+e+f for a, b, c, d, e, f in zip(gene,expression,expression_group1, expression_group2, pvalue, foldchange)]
    ...
    log2cp10k = list(round(arange(0.0, 1.5, 0.100000000000), 2))+list(range(2, max_log2cp10k+1, 1))
    ...
    updatemenus=list([ dict(buttons=list(pvalue_buttons),
                            direction = 'down',
                            pad = {'r': 10, 't': 10},
                            showactive = True,
                            x = 0.2,
    ...
    annotations = list([
        dict(text='FDR: <br> max value', x=0.1, y=1.05, align='left', yref = 'paper', xref = 'paper', showarrow=False ),
        dict(text='absolute logFC: <br> min value', x=0.4, y=1.05, align='left', yref = 'paper', xref = 'paper',  showarrow=False ),
        dict(text="log2cp10k expression: <br> min value", x=0.76, y=1.05, align='left', yref = 'paper', xref = 'paper', showarrow=False),
    ])
...
def get_de(adata,mygroup, demethod='wilcoxon',topnr=5000, logfc=1,padj=0.05):
    ...
    for i in mygroups:
        ...
        dfFC=DataFrame(adata.uns['rank_genes_groups']['logfoldchanges']).head(topnr)[i]
