...
def annotate_cells_clustering(adata,
    ...
    for i in range(0, len(clusters)):
        cluster_annotation.get(new_annotation_label).replace(clusters[i], new_cluster_labels[i], inplace = True)

    #write results back into adata
    adata.obs = adata.obs.merge(cluster_annotation, how = 'outer', left_index = True, right_index = True)
    return(None)
