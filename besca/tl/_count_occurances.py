...
def count_occurance(adata,
    ...
    This function counts the occurance of each label within the specified column (count_variable)
    of an AnnData object and outputs the results to a pandas DataFrame. If add_percentage is true
    it also calculates the occurance of each label as a percentage. Note, percentages have been 
    rounded to the second decimal place after the comma.

    One of the most common use-cases for this function will be to count the occurance of specific
    ...
    add_percentage: `bool` | default = False
      boolian indicator if the occurance of each label as a percentage should be added to
      the dataframe
    ...
    pandas.DataFrame
        ...
        also contains the occurance of each label as a percentage
...
def count_occurance_subset(adata,
                           subset_variable,
                           count_variable='celltype',
                           return_percentage=False):
    """ count occurance of a label in adata.obs after subseting adata object
    
    This function subsets the supplied AnnData object into datasubsets according to the lables contained
    in the subset_variable. For each of the datasubsets, it counts the occurance of each label 
    within the specified column (count_variable) of the adata subset. 

    If return percentage = True then the percentage of each label occurance within a subset is returned.
    ...
    pandas.DataFrame
        Dataframe containing the counts of each label, if add_percentage = True, then the DataFrame
        contains the occurance of each label as a percentage within the datasubset
    ...
    for subset in subsets:
        ...
        all_counts = count_occurance(data, count_variable=count_variable, add_percentage=True)
...
def count_occurance_subset_conditions(adata,
                                      subset_variable,
                                      condition_identifier,
                                      count_variable ='celltype',
                                      return_percentage = False):
    """count occurance of a label for each condition in adata.obs after subseting adata object
    ...
    for subset in subsets:
        ...
        if return_percentage:
            ...
            percentages = count_occurance_subset(data, count_variable = count_variable, subset_variable = condition_identifier, return_percentage=True)
        ...
        #generate dataframe containing counts
            counts = count_occurance_subset(data, count_variable = count_variable, subset_variable = condition_identifier)
