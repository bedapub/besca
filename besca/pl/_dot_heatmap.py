...
def _generate_circle(expression_values, center, radius, ax):
    ...
    """
    slices, labels = ax.pie(expression_values.counts, 
...
def _get_expression_table(adata, gene, color_map, max_expression):
    ...
    if adata.n_obs > 1:
        ...
        expression_values = DataFrame({'counts':[1], 'value':adata.X}, index = adata.obs.index)
        expression_values.sort_values(ascending=True, by='value', axis=0, inplace=True)
        expression_values.index = range(expression_values.shape[0])
...
def dot_heatmap(adata, 
    ...
    if switch_axis:
        ...
        n_cols = len(groups) + 6 #additional columns: 2 columns as spacers, 1 column for group labels, 1 column for colorbar, 1 coloum for dotsize legend
    else:
        n_cols = len(genes) + 6 #additional columns: 2 columns as spacers, 1 column for group labels, 1 column for colorbar, 1 coloum for dotsize legend
    ...
    n_cells['radius'] = [_rescale(val = x, 
                                 in_max= bins['upper_limit_in'].get(bin_id), 
                                 in_min= bins['lower_limit_in'].get(bin_id), 
                                 out_max=bins['upper_limit_out'].get(bin_id), 
                                 out_min=bins['lower_limit_out'].get(bin_id)) for x, bin_id in zip(n_cells.n_cells.tolist(), n_cells.bin.tolist())]
    ...
    if figsize is None:
        ...
        heatmap_height = max([1.5, heatmap_height]) #ensure that we have a certain minimal height even for small number of groups
        height = heatmap_height  + label_height
        width = label_width + heatmap_width + colorbar_width_spacer + colorbar_width + size_legend_spacer + size_legend_width + size_legend_label
    else:
        width, height = figsize
        heatmap_width = width - (colorbar_width_spacer + colorbar_width + size_legend_spacer + size_legend_width + size_legend_label)
    ...
    #determine ratios
    if switch_axis:
        width_ratios = [label_width] + [width_group]*len(groups) + [colorbar_width_spacer, colorbar_width, size_legend_spacer, size_legend_width, size_legend_label]
        height_ratios = [label_height] + [height_gene]*len(genes)
    else:
        width_ratios = [label_width] + [width_gene]*len(genes) + [colorbar_width_spacer, colorbar_width, size_legend_spacer, size_legend_width, size_legend_label]
    ...
    axs = gridspec.GridSpec(nrows=n_rows, ncols=n_cols, wspace=0.02, hspace=0.04,
    ...
    if n_rows < 8:
        ...
        wspace = 10.5 / width
        axs2 = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=axs[1:, n_cols-4], wspace=wspace,
                                                height_ratios=[colorbar_height / height, (height - colorbar_height) / height])
    ...
    text = label_num_cells.text(0.2, 0.1, 'log2cp10k', ha='center', va='bottom', rotation=0)
    
    #add dotsize legend relating to cellnumber in group
    label_num_cells= fig.add_subplot(axs[0, -2])
    label_num_cells.axis('off')
    text = label_num_cells.text(0.2, 0.1, 'number of cells', ha='left', va='bottom', rotation=0)
    ...
    for n in range(n_bins):
        ...
        bin_label.axis('off')
        text = bin_label.text(0.2, 0.5, str(int(bins.upper_limit_in.get(this_bin))), ha='left', va='center', rotation=0)
    ...
    if switch_axis:
        for i in range(sub_num_cols):
            ...
            label.axis('off')
            text = label.text(0.5, 0.2, groups[i], ha='center', va='bottom', rotation = 90)
            for j in range(sub_num_rows):
                data = _get_expression_table(adata_subset, genes[j], color_map= color_map, max_expression = max_expression)
                if data is not None:
...
def dot_heatmap_split(adata, 
    ...
    if n_conditions != 2:
        sys.exit('Can only compare two conditions! Please ensure that adata.obs.get(split_variable) only contains two conditions.')
    ...
    if switch_axis:
        if rotation is None:
            rotation = 90
        n_rows = len(genes) + 1 #additional rows: 1 row for the gene labels
        n_cols = 2*len(groups) + len(groups) - 1 + 8 #additional columns:(groups - 1) X dis_columns between groups, 2 columns as spacers, 1 column for group labels, 1 column for colorbar, 1 coloum for dotsize legend
    else:
        if rotation is None:
            rotation = 0
        n_cols = 2*len(genes) + len(genes) - 1 + 8 #additional columns: (genes - 1) X dis_columns between groups, 2 columns as spacers, 1 column for group labels, 1 column for colorbar, 1 coloum for dotsize legend
        n_rows = len(groups) + 1 #additional rows: 1 row for the gene labels
    ...
    bins = list(set(n_cells_cond1.bin.value_counts().index.tolist() + n_cells_cond2.bin.value_counts().index.tolist()))
    ...
    n_cells_cond1['radius'] = [_rescale(val = x, 
                                 in_max= bins['upper_limit_in'].get(bin_id), 
                                 in_min= bins['lower_limit_in'].get(bin_id), 
                                 out_max=bins['upper_limit_out'].get(bin_id), 
                                 out_min=bins['lower_limit_out'].get(bin_id)) for x, bin_id in zip(n_cells_cond1.n_cells.tolist(), n_cells_cond1.bin.tolist())]


    n_cells_cond2['radius'] = [_rescale(val = x, 
                                 in_max= bins['upper_limit_in'].get(bin_id), 
                                 in_min= bins['lower_limit_in'].get(bin_id), 
                                 out_max=bins['upper_limit_out'].get(bin_id), 
                                 out_min=bins['lower_limit_out'].get(bin_id)) for x, bin_id in zip(n_cells_cond2.n_cells.tolist(), n_cells_cond2.bin.tolist())]
    ...
    if figsize is None:
        ...
        heatmap_height = max([1.5, heatmap_height]) #ensure that we have a certain minimal height even for small number of groups
        height = heatmap_height  + label_height
        width = label_width + heatmap_width + colorbar_width_spacer + colorbar_width + between_colorbar_spacer + colorbar_width + size_legend_spacer + size_legend_width + size_legend_label
    else:
        width, height = figsize
        heatmap_width = width - (label_width + colorbar_width_spacer + colorbar_width + between_colorbar_spacer + colorbar_width + size_legend_spacer + size_legend_width + size_legend_label)
        heatmap_heigth = height - label_height

    if switch_axis:
        width_group = (heatmap_width - dis_pairs*(len(groups)-1)) / (2*len(groups))
        height_gene = heatmap_height / len(genes)
    else:
    ...
    #determine ratios
    if switch_axis:
        width_ratios = [label_width] + [width_group]*2 + [dis_pairs, width_group, width_group]*(len(groups)-1) + [colorbar_width_spacer, colorbar_width, between_colorbar_spacer, colorbar_width, size_legend_spacer, size_legend_width, size_legend_label]
        height_ratios = [label_height] + [height_gene]*len(genes)
    else:
        width_ratios = [label_width] + [width_gene]*2 + [dis_pairs, width_gene, width_gene]*(len(genes)-1) + [colorbar_width_spacer, colorbar_width, between_colorbar_spacer, colorbar_width, size_legend_spacer, size_legend_width, size_legend_label]
        height_ratios = [label_height] + [height_group]*len(groups)
    ...
    if n_rows < 8:
        ...
        colorbar_height = min(5.0, height - label_height)
        wspace = 10.5 / width
        axs2_cond1 = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=axs[2:, -6], wspace=wspace,
                                                height_ratios=[colorbar_height / height, (height - colorbar_height) / height])
        axs2_cond2 = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=axs[2:, -4], wspace=wspace,
                                                height_ratios=[colorbar_height / height, (height - colorbar_height) / height])
    ...
    label.axis('off')
    text = label.text(0.5, 0.05, 'log2cp10k', ha='center', va='bottom', rotation=0, fontsize = 'x-large')

    #add legend for each condition
    label_cond1= fig.add_subplot(axs[1, -6])
    label_cond1.axis('off')
    text = label_cond1.text(0.5, 0.5, conditions[0], ha='center', va='center', rotation=0)

    label_cond2= fig.add_subplot(axs[1, -4])
    label_cond2.axis('off')
    text = label_cond2.text(0.5, 0.5, conditions[1], ha='center', va='center', rotation=0)

    #add dotsize legend relating to cellnumber in group
    label_num_cells= fig.add_subplot(axs[0, -2])
    label_num_cells.axis('off')
    text = label_num_cells.text(0.0, 0.05, 'number of cells', ha='left', va='bottom', rotation=0, fontsize = 'x-large')
    ...
    if switch_axis:
        for j, g in zip(colranges[0], range(len(groups))):
            ...
            label.axis('off')
            text = label.text(0.5, 0.05, groups[g], ha='center', va='bottom', rotation = rotation)
            label.annotate("", xy=(0, 0), xycoords='data',
    ...
    if switch_axis:
        ...
        for j, g in zip(colranges[0], range(len(genes))):
            label = fig.add_subplot(axs[0, j:(j+2)])
            label.axis('off')
            text = label.text(0.5, 0.05, genes[g], ha='center', va='bottom', rotation=rotation)
            label.annotate("", xy=(0, 0), xycoords='data',
                           xytext=(1, 0), textcoords='data',
                           arrowprops=dict(arrowstyle="-",
                                           connectionstyle="arc3,rad=0.") )
...
def dot_heatmap_split_greyscale(adata, 
                                genes, 
                                split_by,
                                group_by = 'louvain',
                                abbreviations = None,
    ...
    if switch_axis:
        if rotation is None:
            rotation = 90
        n_rows = len(genes) + 2 #additional rows: 1 row for the gene labels and 1 for condition labels
        n_cols = 2*len(groups) + len(groups) - 1 + 6 #additional columns:(groups - 1) X dis_columns between groups, 2 columns as spacers, 1 column for group labels, 1 column for colorbar, 2 coloum for dotsize legend
    else:
        if rotation is None:
            rotation = 0
        n_cols = 2*len(genes) + len(genes) - 1 + 6 #additional columns: (genes - 1) X dis_columns between groups, 2 columns as spacers, 1 column for group labels, 1 column for colorbar, 1 coloum for dotsize legend
    ...
    if figsize is None:
        ...
        width, height = figsize
        heatmap_width = width - (label_width + colorbar_width_spacer + colorbar_width  + size_legend_spacer + size_legend_width + size_legend_label)
    ...
    #determine ratios
    if switch_axis:
        width_ratios = [label_width] + [width_group]*2 + [dis_pairs, width_group, width_group]*(len(groups)-1) + [colorbar_width_spacer, colorbar_width, size_legend_spacer, size_legend_width, size_legend_label]
        height_ratios = [label_height, condition_label] + [height_gene]*len(genes)
    else:
        width_ratios = [label_width] + [width_gene]*2 + [dis_pairs, width_gene, width_gene]*(len(genes)-1) + [colorbar_width_spacer, colorbar_width, size_legend_spacer, size_legend_width, size_legend_label]
    ...
    if n_rows < 8:
        ...
        colorbar_height = min(5.0, height - label_height)
        wspace = 10.5 / width
        axs2 = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=axs[2:, -4], wspace=wspace,
                                                height_ratios=[colorbar_height / height, (height - colorbar_height) / height])
    ...
    label_num_cells.axis('off')
    text = label_num_cells.text(0.0, 0.05, 'number of cells', ha='left', va='bottom', rotation=0)
    ...
    for c in range(len(conditions)):
        for j in colranges[c]:
            ...
            label.axis('off')
            text = label.text(0.5, 0.05, abbrev_labels[c], ha='center', va='bottom', rotation = 0)
