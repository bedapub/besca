...
def valOutlier(adata, nmads = 3, rlib_loc = ''):
    ...
    ro.r('dat =  SingleCellExperiment(assays = list(counts=seurat_obj@assays$RNA@counts) )')
...
def scTransform(adata, hvg = False, n_genes = 4000, rlib_loc = ''):
    ...
    if hvg:
        ...
        ro.r('res <- SCTransform(object=seurat_obj, return.only.var.genes = TRUE, do.correct.umi = FALSE, variable.features.n = n_genes)')
